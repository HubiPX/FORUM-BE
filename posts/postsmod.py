...
def get_all_posts(page_nr):
    ...
    volume = 10
    pages = math.ceil(db.session.query(Postsm).count() / volume)

    if pages < page_nr < 1:
        return ''

    offset = (page_nr - 1) * volume  # ile trzeba pominąć
    posts = db.session.query(Postsm, Users).join(Postsm).order_by(Postsm.id.desc()).offset(offset).limit(volume)
