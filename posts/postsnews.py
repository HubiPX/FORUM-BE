...
def get_all_posts(page_nr):
    ...
    volume = 10
    pages = math.ceil(db.session.query(Postsnews).count() / volume)

    if pages < page_nr < 1:
        return ''

    offset = (page_nr - 1) * volume  # ile trzeba pominąć
    posts = db.session.query(Postsnews, Users).join(Postsnews).order_by(Postsnews.id.desc()).offset(offset).limit(volume)
